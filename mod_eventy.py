        ...
        if not df_aktywne.empty:
                    ...
                    with tab_container:
                        if df_subset.empty:
                            ...
                            for index, row in df_subset.iterrows():
                                ...
                                if wymaga_cmr(row): braki_tagi_html += "<span class='tag-zen-red'>🚨 WYSTAW CMR</span>"
                                if not is_sqm_row:
                                ...
                                elif is_past_end_date(row): badge_class, faza_wyswietlana = "cr-badge zamkniete", "ZAMKNIĘTE (AUTO)"
                                else:
                                ...
                                # Odseparowanie Dopisku od Głównego Eventu do wyświetlania
                                raw_name = str(row.get('Nazwa_Targow', '-'))
                                if " | " in raw_name:
                                    base_name, dopisek_name = raw_name.split(" | ", 1)
                                    display_name = f"{base_name} <span style='font-size: 12px; font-weight: 600; color: #BA4949;'>| {dopisek_name}</span>"
                                else:
            ...
            with col_detale:
                if st.session_state["wybrany_event_id"] and not df_widok[df_widok["ID_Zlecenia"] == st.session_state["wybrany_event_id"]].empty:
                    ...
                    # Dekodowanie Dopisku na potrzeby nagłówka
        ...
        with st.form("form_event_pro", clear_on_submit=True):
            ...
            if st.form_submit_button("🚀 Zainicjuj Zlecenie Systemowe"):
                if baza_targow == "-- Wybierz lub dodaj w słowniku --" or not przewoznik:
                    ...
                    kolumny = [k for k in df.columns if k != 'sheet_row']
                    db.append_data("DB_Eventy", [str(nowy_wiersz_z_id.get(k, "")) for k in kolumny])
                    st.success("🎉 Zlecenie zapisane w bazie chmurowej (Bezpieczny zapis)!"); st.rerun()
