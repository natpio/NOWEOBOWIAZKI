...
def is_past_end_date(row, dzisiaj_date):
    faza = str(row.get('Faza_Procesu', '')).lower()
    if "zamknięte" in faza: return True
    if "planowanie" in faza or "inicjacja" in faza or "załadunek" in faza: return False
    ...
    if "[Rozładunki:" in notatki:
        try: 
            ...
            if roz_raw: roz = roz_raw.split(",")[-1].strip()
        except: pass
...
def render_event_details(wybrany_id, df_widok, df, df_miejsca, df_przewoznicy, opcje_lokalizacji, lista_eventow_slownik, sh):
    ...
    if " | " in raw_name:
        base_name, dopisek_name = raw_name.split(" | ", 1)
    else:
        base_name, dopisek_name = raw_name, ""
    ...
    if dopisek_name:
        st.markdown(f"<h3 style='color: #E2DCD3; margin-top: 0;'>{base_name} <br><span style='color: #BA4949; font-size: 20px;'>{dopisek_name}</span></h3>", unsafe_allow_html=True)
    else:
        st.markdown(f"<h3 style='color: #E2DCD3; margin-top: 0;'>{raw_name}</h3>", unsafe_allow_html=True)
    ...
    with c_dup:
        if st.button("📋 Klonuj", key=f"clone_{dane_eventu.get('ID_Zlecenia', '')}", use_container_width=True):
            ...
            nowy_wiersz['CMR_Gotowe'], nowy_wiersz['Nr_CMR'] = "NIE", "" 
            ...
            nowy_wiersz['Nr_Faktury'], nowy_wiersz['Data_Platnosci'] = "N/A" if is_sqm_clone else "", "N/A" if is_sqm_clone else ""
    ...
    plik_img = "ftl.png" if "ftl" in typ_pojazdu_lower else "bus.png" if "bus" in typ_pojazdu_lower else "van.png" if "van" in typ_pojazdu_lower else "solowka.png" if "sol" in typ_pojazdu_lower else "default.png"
    ...
    braki_zlecenia = []
    if check_wymaga_cmr(dane_eventu): braki_zlecenia.append("📝 <b>Wystawić dokument CMR</b> dla kierowcy")
    if not is_sqm:
        if dane_eventu.get("CMR_Podpisane_POD") == "NIE": braki_zlecenia.append("📄 <b>Odzyskać podpisane CMR (POD)</b>")
        if dane_eventu.get("Faktura_Oplacona") == "NIE": braki_zlecenia.append("💰 <b>Opłacić fakturę</b> zewnętrznego przewoźnika")
        if dane_eventu.get("PP_Otrzymane") == "NIE": braki_zlecenia.append("💳 Zdobyć <b>Potwierdzenie Przelewu (PP)</b>")
        if str(dane_eventu.get("Nr_Faktury", "")).strip() in ["", "None"]: braki_zlecenia.append("🔢 Wprowadzić <b>Numer Faktury Zewn.</b>")

    if braki_zlecenia:
        lista_brakow = "".join([f"<li style='margin-bottom: 5px;'>{b}</li>" for b in braki_zlecenia])
        st.markdown(f'<div style="background: rgba(186, 73, 73, 0.05); border: 1px solid rgba(186, 73, 73, 0.3); padding: 15px 20px; border-radius: 8px; margin: 15px 0;"><h4 style="color: #BA4949; margin: 0 0 10px 0; font-size: 12px; text-transform: uppercase;">⚠️ Wymagane Akcje:</h4><ul style="color: #E2DCD3; font-size: 13px; margin: 0; padding-left: 20px;">{lista_brakow}</ul></div>', unsafe_allow_html=True)
    ...
    with det_info:
        with st.form(key=f"edit_all_{dane_eventu['ID_Zlecenia']}"):
            ...
            with c_ed1:
                ...
                u_baza_targow = st.selectbox("Główny Event (Słownik) *", ["-- Wybierz lub dodaj w słowniku --"] + lista_eventow_slownik, index=lista_eventow_slownik.index(base_name)+1 if base_name in lista_eventow_slownik else 0)
                u_dopisek = st.text_input("Dopisek / Sub-projekt (np. PRZERZUT)", value=dopisek_name)
                
                akt_miejsce = str(dane_eventu.get('Miejsce_Przeznaczenia', ''))
                if akt_miejsce in opcje_lokalizacji:
                    idx_m, init_m_man = opcje_lokalizacji.index(akt_miejsce), ""
                else:
                    idx_m, init_m_man = opcje_lokalizacji.index("INNE (wpisz ręcznie)") if "INNE (wpisz ręcznie)" in opcje_lokalizacji else 0, akt_miejsce
                ...
                u_faza = st.selectbox("Faza Procesu", fazy_lista, index=fazy_lista.index(akt_faza) if akt_faza in fazy_lista else 0)
...
def render_new_event_form(df, df_miejsca, df_przewoznicy, lista_eventow_slownik, opcje_lokalizacji):
    ...
    with st.form("form_event_pro", clear_on_submit=True):
        ...
        val_c_auto_nr, val_c_kierowca, val_waga, val_stawka_final = "", "", 1000, 0.0
        val_data_zal, val_data_odbior_pelnych = None, None
        ...
        if import_data:
            ...
            if dt_zak and dt_zak not in ["nan", "None"]:
                ...
                try: val_data_odbior_pelnych = datetime.datetime.strptime(dt_zak, "%Y-%m-%d").date()
                except: pass
            ...
            if nazwa_przew and nazwa_przew != "nan":
                ...
                if val_nazwa_przewoznika in lista_firm_db: val_zrodlo = "Przewoźnik stały (Baza)"
                    
            dest = str(import_data.get('Miejsce_Przeznaczenia', '')).strip()
            if dest and dest != "nan": val_miejsca_rozladunku_raw = [dest]
        ...
        with f_col1:
            ...
            baza_targow = st.selectbox("Główny Event (Ze słownika) *", ["-- Wybierz lub dodaj w słowniku --"] + lista_eventow_slownik)
            dopisek = st.text_input("Dopisek / Sub-projekt (np. PRZERZUT, HALA 4)", placeholder="Opcjonalnie...")
            ...
            data_zaladunku_nowa = st.date_input("Data Załadunku", value=val_data_zal)
            
        with f_col2:
            przewoznik = st.text_input("Przewoźnik / Firma Transportowa *", value=val_nazwa_przewoznika)
            kierowca = st.text_input("Imię i Nazwisko Kierowcy (do CMR)", value=val_c_kierowca)
            nr_rejestracyjny = st.text_input("Nr Rejestracyjny Pojazdu (do CMR)", value=val_c_auto_nr)
            waga = st.number_input("Waga (kg)", min_value=0, step=100, value=val_waga)
        ...
        data_powrotu_baza = r_pow.date_input("Powrót do bazy (Zakończenie):", value=val_data_odbior_pelnych)
        ...
        if typ_transportu == "Zewnętrzny":
            st.markdown("<hr style='border-color: rgba(255,255,255,0.05);'><h4 style='color: #C5A880;'>🏁 Finanse i Dowód Dostawy (POD)</h4>", unsafe_allow_html=True)
            ...
            with e_col1: koszt_transportu = st.number_input("Koszt Transportu (€)", min_value=0.0, value=val_stawka_final, step=50.0)
            with e_col2: nr_faktury = st.text_input("Nr Faktury Przewoźnika")
        else:
            st.info("💡 Wybrano Flotę Własną SQM. Sekcja finansowa została pominięta.")
        ...
        if st.form_submit_button("🚀 Zainicjuj Zlecenie Systemowe"):
            if baza_targow == "-- Wybierz lub dodaj w słowniku --" or not przewoznik:
                ...
                final_nazwa_targow = baza_targow if not dopisek.strip() else f"{baza_targow} | {dopisek.strip()}"
...
def render_lista_eventow(df_aktywne, braki_cmr, braki_pod, braki_faktury, dzisiaj_date, df, df_miejsca, df_przewoznicy, opcje_lokalizacji, lista_eventow_slownik, sh):
    ...
    wyszukiwarka = st.text_input("Wyszukiwarka", placeholder="🔍 Wpisz nazwę targów, przewoźnika, ID zlecenia, fakturę (rozdzielaj przecinkiem)...", label_visibility="collapsed")
    ...
    if st.session_state["filtr_eventow"] == "BrakCMR" and not df_widok.empty:
        df_widok = df_widok[df_widok.apply(check_wymaga_cmr, axis=1).astype(bool)]
    ...
    with col_lista:
        ...
        if df_widok.empty:
            ...
            mask_zamkniete = df_widok.apply(lambda r: is_past_end_date(r, dzisiaj_date), axis=1).astype(bool)
            ...
            with tab_container:
                if df_subset.empty:
                    ...
                    for index, row in enumerate(df_subset.to_dict('records')):
                        ...
                        if not is_sqm_row:
                            if str(row.get('CMR_Podpisane_POD', '')) == 'NIE': braki_tagi_html += "<span class='tag-zen-orange'>📄 BRAK POD</span>"
                            if str(row.get('Faktura_Oplacona', '')) == 'NIE': braki_tagi_html += "<span class='tag-zen-red'>💰 NIEOPŁACONE</span>"
                            if str(row.get('PP_Otrzymane', '')) == 'NIE': braki_tagi_html += "<span class='tag-zen-blue'>💳 BRAK PP</span>"
                        ...
                        raw_name = str(row.get('Nazwa_Targow', '-'))
                        if " | " in raw_name:
                            b_name, d_name = raw_name.split(" | ", 1)
                            display_name = f"{b_name} <span style='font-size: 12px; font-weight: 600; color: #BA4949;'>| {d_name}</span>"
                        else:
                            ...
                            <div class="custom-row" style="margin-bottom: 5px; padding: 15px 20px; flex-direction: column;">
                                <div style="display: flex; width: 100%; justify-content: space-between;">
                                    <div class="cr-col" style="width: 40%;">
                                        <div class="cr-title" style="font-size: 18px; color: #050A15 !important; font-weight: 800; margin-bottom: 2px;">{display_name}</div>
...
def render(sh):
    ...
    lista_eventow_slownik = df_etapy['Nazwa_Targow'].dropna().tolist() if not df_etapy.empty else []
    ...
    if not df_aktywne.empty:
        ...
        is_sqm = df_aktywne['Typ_Transportu'] == 'Własny SQM'
        is_bus_van = df_aktywne['Typ_Pojazdu'].fillna('').str.lower().str.contains('bus|van', regex=True)
        braki_cmr = int((cmr_nie & ~(is_sqm & is_bus_van)).sum())
        braki_pod = int((df_aktywne["CMR_Podpisane_POD"] == "NIE").sum())
        braki_faktury = int((df_aktywne["Faktura_Oplacona"] == "NIE").sum())
    ...
    with tab_podglad:
        if not df_aktywne.empty:
            render_lista_eventow(df_aktywne, braki_cmr, braki_pod, braki_faktury, dzisiaj_date, df, df_miejsca, df_przewoznicy, opcje_lokalizacji, lista_eventow_slownik, sh)
        else:
            st.info("Brak aktywnych eventów.")
