...
class PRO_TransportOrder(FPDF):
  ...
  def header(self):
    ...
    self.cell(
        105,
        5,
        pdf_sanitize("SQM Prosta Spółka Akcyjna | Logistics Department"),
        ln=True,
        align="R",
    )
    self.ln(15)
  ...
  def footer(self):
    ...
    self.cell(
        0,
        5,
        pdf_sanitize(f"www.sqm.pl   |   {email}   |   {telefon}"),
        ln=True,
        align="C",
    )
    self.cell(
        0,
        5,
        pdf_sanitize("Document generated by SQM HUB / Vorteza PRO"),
        ln=True,
        align="C",
    )
...
def generate_pro_pdf(dane):
  ...
  pdf.cell(
      60,
      8,
      pdf_sanitize(f" {datetime.now().strftime('%d.%m.%Y')}"),
      border=0,
      fill=True,
  )
  pdf.ln(12)
  ...
  draw_row(
      "VEHICLE & DRIVER / AUTO I KIEROWCA:",
      dane["auto"] if dane["auto"] else "TBA / Do podania",
  )
  ...
  if dane["typ_zlecenia"] == "Pełny event":
    ...
    draw_row(
        "UNLOADING DATE / DATA ROZŁADUNKU:", dane["data_roz"], border_b=False
    )
  pdf.ln(4)
  ...
  pdf.set_xy(120, sy)
  pdf.set_fill_color(25, 118, 210)
  pdf.rect(120, sy, 25, 25, "F")
  pdf.set_xy(145, sy)
  pdf.set_fill_color(25, 118, 210)
  pdf.rect(145, sy, 55, 25, "F")
  pdf.set_xy(145, sy + 3)
  pdf.set_font("Arial", "B", 8)
  pdf.set_text_color(255, 255, 255)
  pdf.cell(50, 5, pdf_sanitize("TOTAL NET RATE / KWOTA NETTO"), ln=True)
  pdf.set_xy(145, sy + 10)
  ...
  pdf.set_font("Arial", "B", 8)
  pdf.set_text_color(100, 100, 100)
  pdf.cell(55, 5, pdf_sanitize("CARGO TYPE / RODZAJ TOWARU:"), border=0)
  pdf.set_font("Arial", "B", 10)
  pdf.set_text_color(40, 40, 40)
  pdf.set_xy(65, sy)
  pdf.multi_cell(
      50, 5, pdf_sanitize("Exhibition Structures / AV Equipment")
  )
  ...
  pdf.set_font("Arial", "B", 8)
  pdf.set_text_color(100, 100, 100)
  pdf.cell(55, 5, pdf_sanitize("PAYMENT / PŁATNOŚĆ:"), border=0)
  pdf.set_font("Arial", "B", 10)
  pdf.set_text_color(40, 40, 40)
  pdf.set_xy(65, pdf.get_y())
  pdf.cell(
      ...
      5,
      pdf_sanitize(
          f"{dane['termin_dni']} dni / days ({dane['data_platnosci']})"
      ),
  )
...
def generate_cmr_excel(dane):
  ...
  if not os.path.exists(szablon_path):
    raise FileNotFoundError(
        f"Nie znaleziono pliku szablonu: {szablon_path}. Umieść go w katalogu"
        " głównym."
    )

  wb = openpyxl.load_workbook(szablon_path)
  nadawca_tekst = (
      "SQM Prosta Spółka Akcyjna ;\nul. Poznańska 165, 62-052 Komorniki,\nNIP:"
      " 7792361182"
  )
...
# --- POBIERANIE DANYCH Z CACHEM (OCHRONA PRZED BŁĘDEM 429) ---
...
def render(sh):
  ...
  with tab1:
    ...
    with c1:
      tryb_pracy = st.radio(
          "Wybierz tryb pracy:",
          ["Nowe Zlecenie", "Edycja Istniejącego Zlecenia"],
          horizontal=True,
      )
    with c2:
      kategoria_zlecenia = st.radio(
          ...
          [
              "Zlecenie Poboczne (Eksport do rejestru)",
              "Zlecenie Eventowe (Pomiń rejestr poboczny)",
          ],
          horizontal=True,
      )
    ...
    domyslny_tekst = (
        "Parking strzeżony, pasy zabezpieczające; załadować po długości, casy"
        " nie mogą leżeć, kłódka / Guarded parking, safety belts; load"
        " lengthwise, cases cannot lie down, safe lock."
    )
    ...
    if tryb_pracy == "Edycja Istniejącego Zlecenia":
      if not df_cargo.empty:
        ...
        wybrane_zlecenie_nr = st.selectbox(
            "🎯 Wybierz numer zlecenia do korekty/edycji:", lista_nr
        )

        idx_pd = df_zlecenia[
            df_zlecenia["Numer zlecenia"] == wybrane_zlecenie_nr
        ].index[0]
        ...
        val_typ_zlecenia = (
            "Pełny event"
            if "TARGI" in str(r_edit.get("Typ", ""))
            or "CYKL:" in str(r_edit.get("Uwagi / Instrukcje", ""))
            else "Tylko dostawa"
        )

        try:
          val_data_zal = datetime.strptime(
              str(r_edit.get("Data załadunku", r_edit.iloc[6])), "%Y-%m-%d"
          ).date()
        except:
          pass
        try:
          val_data_roz = datetime.strptime(
              str(r_edit.get("Data rozładunku", r_edit.iloc[7])), "%Y-%m-%d"
          ).date()
        except:
          pass
        ...
        if "WART: " in uwagi_baza:
          try:
            val_wartosc_towaru = int(
                uwagi_baza.split("WART: ")[1].split(" PLN")[0]
            )
          except:
            pass
        ...
        if (
            not df_przewoznicy.empty
            and "Skrócona Nazwa" in df_przewoznicy.columns
        ):
          r_p = df_przewoznicy[
              df_przewoznicy["Skrócona Nazwa"] == val_nazwa_przewoznika
          ]
          if not r_p.empty:
            r = r_p.iloc[0]
            val_detale_przewoznika = (
                f"{str(r.get('Pełna Nazwa', ''))}\n{str(r.get('Ulica i numer',"
                f" ''))}\n{str(r.get('Kod pocztowy i Miasto', ''))},"
                f" {str(r.get('Kraj', 'Polska'))}\nNIP:"
                f" {str(r.get('NIP', ''))}"
            ).strip()
            val_zrodlo = "Przewoźnik stały (Baza)"
          else:
    ...
    with st.expander("🔍 Przeglądaj i wyszukaj miejsca z Bazy Lokalizacji"):
      wyszukiwana_fraza = st.text_input(
          ...
          placeholder="np. Messe Berlin...",
      )
      if not df_miejsca.empty:
        if wyszukiwana_fraza:
          maska = df_miejsca.astype(str).apply(
              lambda row: row.str.contains(
                  wyszukiwana_fraza, case=False, na=False
              ).any(),
              axis=1,
          )
          st.dataframe(
              df_miejsca[maska], use_container_width=True, hide_index=True
          )
        else:
          st.dataframe(df_miejsca, use_container_width=True, hide_index=True)

    with st.expander(
        "➕ Brak miejsca na liście? Dodaj nową lokalizację do Słownika"
    ):
      with st.form("form_nowe_miejsce", clear_on_submit=True):
        nowa_nazwa_lista = st.text_input(
            "Nazwa skrócona (do listy wyboru):*",
            placeholder="np. BERLIN, DE - Messe Berlin",
        )
        ...
        if st.form_submit_button("💾 Zapisz lokalizację w bazie"):
          if nowa_nazwa_lista.strip():
            ...
            nowy_wiersz = [
                slownik_nowego.get(kol, "") for kol in kolumny_miejsca
            ]
    ...
    lista_eventow = (
        df_projekty["Nazwa Eventu"].dropna().unique().tolist()
        if not df_projekty.empty
        else ["Brak"]
    )
    lista_miejsc_baza = (
        df_miejsca["Nazwa do listy"].tolist() if not df_miejsca.empty else []
    )
    opcje_lokalizacji = (
        ["Magazyn SQM Komorniki"] + lista_miejsc_baza + ["INNE (wpisz ręcznie)"]
    )

    tryb_idx = ["Tylko dostawa", "Pełny event"].index(val_typ_zlecenia)
    typ_zlecenia = st.radio(
        "Tryb operacji:",
        ["Tylko dostawa", "Pełny event"],
        index=tryb_idx,
        horizontal=True,
    )
    ...
    with st.container(border=True):
      st.markdown(
          "<p style='color: #C5A880; font-weight: 700; margin-bottom: 5px;'>1."
          " Harmonogram Zlecenia</p>",
          unsafe_allow_html=True,
      )
      waga = st.number_input(
          "Waga ładunku (kg):", min_value=100, step=100, value=val_waga
      )
      ...
      if typ_zlecenia == "Pełny event":
        h1, h2 = st.columns(2)
        data_emp_in = h1.date_input(
            "Odbiór pustych (Empties):", val_data_emp_in
        )
        data_emp_out = h2.date_input(
            "Data załadunku powrotnego:", val_data_emp_out
        )
      else:
    ...
    with st.container(border=True):
      st.markdown(
          "<p style='color: #C5A880; font-weight: 700; margin-bottom: 5px;'>2."
          " Wybór Przewoźnika i Płatności</p>",
          unsafe_allow_html=True,
      )
      zrodlo_idx = (
          [
              "Przewoźnik stały (Baza)",
              "Przewoźnik z giełdy (Jednorazowy)",
          ].index(val_zrodlo)
          if val_zrodlo
          in [
              "Przewoźnik stały (Baza)",
      ...
      zrodlo = st.radio(
          "Sposób wyboru podwykonawcy:",
          ["Przewoźnik stały (Baza)", "Przewoźnik z giełdy (Jednorazowy)"],
          index=zrodlo_idx,
          horizontal=True,
      )
      ...
      if zrodlo == "Przewoźnik stały (Baza)":
        ...
        lista_cennikowa = (
            df_przewoznicy["Skrócona Nazwa"].dropna().tolist()
            if not df_przewoznicy.empty
            else []
        )
        if (
            tryb_pracy == "Edycja Istniejącego Zlecenia"
            and val_nazwa_przewoznika not in lista_cennikowa
        ):
        ...
        nazwa_przewoznika = f1.selectbox(
            "Wybierz partnera ze słownika:",
            ["Wybierz..."] + lista_cennikowa,
            index=def_p_idx,
        )
        ...
        if nazwa_przewoznika != "Wybierz...":
          if (
            ...
            row_p = df_przewoznicy[
                df_przewoznicy["Skrócona Nazwa"] == nazwa_przewoznika
            ]
            if not row_p.empty:
              r = row_p.iloc[0]
              detale_przewoznika = (
                  f"{str(r.get('Pełna Nazwa', nazwa_przewoznika))}\n{str(r.get('Ulica i numer', ''))}\n{str(r.get('Kod pocztowy i Miasto', ''))}, {str(r.get('Kraj', 'Polska'))}\nNIP: {str(r.get('NIP', ''))}"
              ).strip()
            else:
        ...
        stawka_final = f2.number_input(
            "Stawka Total:", value=float(val_stawka_final)
        )
        currency_idx = (
            ["EUR", "PLN"].index(val_waluta)
            if val_waluta in ["EUR", "PLN"]
            else 0
        )
        waluta = f3.selectbox("Waluta:", ["EUR", "PLN"], index=currency_idx)
        postoj = (
            f4.number_input("Postój:", value=float(val_postoj))
            if typ_zlecenia == "Pełny event"
            else 0.0
        )
      else:
        nazwa_przewoznika = st.text_input(
            "Nazwa firmy z giełdy:", value=val_nazwa_przewoznika
        )
        detale_przewoznika = st.text_area(
            "Pełne dane (Adres, NIP do zlecenia):", value=val_detale_przewoznika
        )
        f1, f2, f3 = st.columns(3)
        stawka_final = f1.number_input(
            "Stawka netto:", min_value=0.0, value=float(val_stawka_final)
        )
        currency_idx = (
            ["EUR", "PLN"].index(val_waluta)
            if val_waluta in ["EUR", "PLN"]
        ...
        postoj = (
            f3.number_input("Postój:", min_value=0.0, value=float(val_postoj))
            if typ_zlecenia == "Pełny event"
            else 0.0
        )

      t1, t2 = st.columns([1, 2])
      termin_dni = t1.number_input(
          "Termin płatności (dni od rozładunku):",
          min_value=0,
          max_value=120,
          value=val_termin_dni,
          step=1,
      )
      data_platnosci = data_roz + timedelta(days=termin_dni)
      t2.info(
          f"📅 Wyliczona data zapłaty:"
          f" **{data_platnosci.strftime('%d.%m.%Y')}**"
      )

    with st.container(border=True):
      st.markdown(
          "<p style='color: #C5A880; font-weight: 700; margin-bottom: 5px;'>3."
          " Logistyka Miejsc</p>",
          unsafe_allow_html=True,
      )

      def_proj_idx = (
          lista_eventow.index(val_projekt)
          if val_projekt in lista_eventow
          else 0
      )
      projekt = st.selectbox(
          "Przypisz do Projektu (Opcjonalnie):",
          lista_eventow,
          index=def_proj_idx,
      )
      ...
      with l1:
        if val_z_sel in opcje_lokalizacji:
          ...
          idx_z = (
              opcje_lokalizacji.index("INNE (wpisz ręcznie)")
              if "INNE (wpisz ręcznie)" in opcje_lokalizacji
              else 0
          )
          init_z_man = val_z_sel

        z_sel = st.selectbox(
            "Miejsce startu (Załadunek):", opcje_lokalizacji, index=idx_z
        )
        z_man = (
            st.text_input("Adres startu (ręcznie):", value=init_z_man)
            if z_sel == "INNE (wpisz ręcznie)"
            else ""
        )
      ...
      with l2:
        if typ_zlecenia == "Tylko dostawa":
          ...
          init_len = (
              len(val_miejsca_rozladunku_raw)
              if len(val_miejsca_rozladunku_raw) > 0
              else 1
          )
          liczba_punktow = st.number_input(
              "Liczba miejsc rozładunku:",
              min_value=1,
              max_value=10,
              value=int(init_len),
              step=1,
          ...
          for i in range(int(liczba_punktow)):
            def_r_item = (
                val_miejsca_rozladunku_raw[i]
                if i < len(val_miejsca_rozladunku_raw)
                else "Wybierz..."
            )
            ...
            r_m = (
                st.text_input(
                    f"Adres DROP {i+1} (ręcznie):",
                    value=init_r_man,
                    key=f"r_man_{i}",
                )
                if r_s == "INNE (wpisz ręcznie)"
                else ""
            )
            miejsca_rozladunku.append((r_s, r_m))
        else:
          def_r_item = (
              val_miejsca_rozladunku_raw[0]
              if len(val_miejsca_rozladunku_raw) > 0
              else "Wybierz..."
          )
          if def_r_item in opcje_lokalizacji:
          ...
          r_m = (
              st.text_input("Adres celu (ręcznie):", value=init_r_man)
              if r_s == "INNE (wpisz ręcznie)"
              else ""
          )
          miejsca_rozladunku.append((r_s, r_m))
    ...
    with st.container(border=True):
      st.markdown(
          "<p style='color: #C5A880; font-weight: 700; margin-bottom: 5px;'>4."
          " Realizacja i Dodatkowe Uwagi</p>",
          unsafe_allow_html=True,
      )
      d_auto, d_wart = st.columns(2)
      c_auto = d_auto.text_input(
          "Auto / Kierowca:",
          value=val_c_auto,
          placeholder="np. PO 12345 / Jan Kowalski",
      )
      wartosc_towaru = d_wart.number_input(
          "Wymagana Gwarancja OCP / Wartość towaru (PLN):",
          min_value=0,
          value=val_wartosc_towaru,
      )

      u1, u2 = st.columns([3, 1])
      instrukcje = u1.text_area(
          "Instrukcje dodatkowe na Zlecenie:", value=val_instrukcje, height=80
      )

      podpis_idx = (
          ["PD", "PK"].index(val_podpis) if val_podpis in ["PD", "PK"] else 0
      )
      podpis = u2.radio(
          "Podpis Koordynatora:", ["PD", "PK"], index=podpis_idx, horizontal=True
      )

    btn_label = (
        "⚡ ZAPISZ ZMIANY I REGENERUJ DOKUMENTY"
        if tryb_pracy == "Edycja Istniejącego Zlecenia"
        else "⚡ GENERUJ I ZAPISZ ZLECENIE PRO"
    )
      ...
      if not nazwa_przewoznika or nazwa_przewoznika == "Wybierz...":
        ...
        with st.spinner("Generowanie dokumentów i aktualizacja chmury..."):
          ...
          def build_full_address(place_name, manual_addr, df):
            ...
            if df is not None and not df.empty:
              ...
              if not row.empty:
                r = row.iloc[0]
                return (
                    f"{r.get('Nazwa pełna / Firma', place_name)}\n{r.get('Ulica"
                    f" i numer', '')}\n{r.get('Kod pocztowy', '')}"
                    f" {r.get('Miasto', '')}, {r.get('Kraj', '')}"
                )
            return place_name
          ...
          if len(lista_roz_pdf) > 1:
            full_roz_pdf = "\n\n".join([
                f"DROP {idx+1}:\n{tekst}"
                for idx, tekst in enumerate(lista_roz_pdf)
            ])
          else:
            full_roz_pdf = lista_roz_pdf[0]

          historia_cyklu = (
              f"CYKL: {data_zal} -> {data_roz}"
              + (
                  f" | EMP: {data_emp_in} | POWRÓT: {data_emp_out}"
                  if typ_zlecenia == "Pełny event"
                  else ""
              )
          )
          pelne_uwagi_db = (
              f"AUTO: {c_auto} || WART: {wartosc_towaru} PLN ||"
              f" {historia_cyklu} || {instrukcje}"
          )
          ...
          paczka_pdf = {
              ...
              "data_platnosci": data_platnosci.strftime("%d.%m.%Y"),
          }
          ...
          if operacja_sukces:
            ...
            if tryb_pracy == "Edycja Istniejącego Zlecenia":
              st.success(
                  f"🎉 Zlecenie {nr_zlecenia} zostało pomyślnie zmodyfikowane w"
                  " bazie danych!"
              )
            else:
              st.success(
                  f"✅ Zlecenie {nr_zlecenia} zostało wygenerowane i zapisane"
                  " pomyślnie!"
              )
  ...
  with tab2:
    st.markdown(
        '<h3 style="color: #E2DCD3; font-family: \'Shippori Mincho\','
        ' serif;">Aktywne Zlecenia PRO</h3>',
        unsafe_allow_html=True,
    )
      ...
      if not df_pro.empty:
        ...
        if df_pro.empty:
          ...
          for index, row in df_pro.iterrows():
            ...
            st.markdown(
                '<hr style="border-color: rgba(197, 168, 128, 0.1); margin: 5px'
                ' 0 15px 0;">',
                unsafe_allow_html=True,
            )
      else:
