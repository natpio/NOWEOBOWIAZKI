...
def render(sh):
    ...
    def render_order_list(df_subset, search_query, empty_msg):
        ...
        for index, row in df_subset.iterrows():
            ...
            img_html = ""
            if b64_batter:
                # Obrys bejsbolisty w tle (8% krycia) - nie zaburza czytelności
            ...
            # Rozbity na linijki HTML - z silnymi tagami color !important, przed wysłaniem ucinamy enter'y
            row_html = f"""
            <div class="custom-row">
                {img_html}
                <div style="display: flex; width: 100%; position: relative; z-index: 2;">
                    <div class="cr-col" style="flex: 2.5; padding-right: 20px;">
                        <div class="cr-title" style="color: #050A15 !important;">{nr_zlecenia_wyswietl}</div>
                        <div class="cr-text" style="color: #1A2530 !important;">🚛 <b style="color: #1A2530 !important;">Przewoźnik:</b> <strong style="color: #990000 !important;">{row.get('Przewoźnik', 'Brak')}</strong></div>
                        <div class="cr-text" style="color: #4A5568 !important;">👤 <b>Opis:</b> <i style="color: #4A5568 !important;">{row.get('Opis Ładunku / Trasy', '---')}</i></div>
                    </div>
                    <div class="cr-col" style="flex: 1.5; border-left: 2px dashed rgba(186, 73, 73, 0.25); padding-left: 25px; justify-content: center;">
                        <div class="cr-text" style="color: #1A2530 !important;">📅 Zał: <span style="font-weight: 700; color: #050A15 !important;">{row.get('Data Załadunku', '---')}</span></div>
                        <div class="cr-text" style="color: #1A2530 !important;">🏁 Rozł: <span style="font-weight: 700; color: #050A15 !important;">{row.get('Data Rozładunku', '---')}</span></div>
                        <div class="cr-text" style="color: #1A2530 !important;">💲 Płatność: <strong style="color: #990000 !important;">{row.get('Data Płatności', '---')}</strong></div>
                        <div class="status-badge">{status_val}</div>
                    </div>
                    <div class="cr-col" style="flex: 1.2; align-items: flex-end; justify-content: center; flex-direction: row; gap: 8px; padding-right: 80px;">
                        {action_buttons}
                    </div>
                </div>
            </div>
